...
string lavalette_map = "data/lavalette.txt";

vector<double> simulateSequencesAndReturnSummaryStatistics(size_t randRootLength,
	Vdouble randInsertionParams,
	Vdouble randDeletionParams,
	double randInsertRatio,
	double randDeletionRatio,
	bool isBurnIn) {
	Simulator sim(randRootLength, randInsertionParams, randDeletionParams, randInsertRatio, randDeletionRatio);
...
Vdouble generateLengthDistributionParameters(DistTable* distTable, string distType){
	Vdouble distParams;
	if (SpartaABC_options::_lengthDistribution == "zipf"){
		double aParam = 1.0;
		if (distType == "insertion") {
			aParam = getRandDoubleParamVal(SpartaABC_options::_priorDistTypeA, SpartaABC_options::_minAIVal, SpartaABC_options::_maxAIVal);
		}
		if (distType == "deletion") {
			aParam = getRandDoubleParamVal(SpartaABC_options::_priorDistTypeA, SpartaABC_options::_minADVal, SpartaABC_options::_maxADVal);
		}
		distParams.push_back(aParam);
	}
	if (SpartaABC_options::_lengthDistribution == "lavalette"){
		// lookup in table and interpolate
		...
		double meanParam = getRandDoubleParamVal(SpartaABC_options::_priorDistTypeA, SpartaABC_options::_minCLavalette, SpartaABC_options::_maxCLavalette);
		double cParam = distTable->interpolate(aParam, meanParam);
...
int main(int argc, const char * argv[])
{
	...
	if(SpartaABC_options::_numDistributionParams == 1){
		resFile << "input_msa\tN/A\tN/A\tN/A\tN/A\tN/A";
	}
	if(SpartaABC_options::_numDistributionParams == 2){
		resFile << "input_msa\tN/A\tN/A\tN/A\tN/A\tN/A\tN/A\tN/A";
	}
	for (size_t i = 0; i < summStatsReal.size(); i++)
	{
	...
	if(SpartaABC_options::_predefinedMode) {
		
		cout << "predef mode!" << endl;
		exit(0);
	}
	
	vector<bool> model_choice = current_model_choice(SpartaABC_options::_modelType);

	// TODO: add support for every distribution table.
	DistTable * distribtutionTable = NULL;//new DistTable(lavalette_map);
	// distribtutionTable->initTable();
	...
	while(totalNumberOfSimulations < totalNumberOfSpartaTests)
	{
		...
		Vdouble	lengthDistParamsInsertion = generateLengthDistributionParameters(distribtutionTable, "insertion");
		Vdouble	lengthDistParamsDeletion = generateLengthDistributionParameters(distribtutionTable, "deletion");
		...
		double randDeletionRatio = getRandDoubleParamVal(SpartaABC_options::_priorDistTypeIR, SpartaABC_options::_minDRVal, SpartaABC_options::_maxDRVal);
		while (randInsertRatio == 0 && randDeletionRatio == 0) {
			 ...
			 randDeletionRatio =  getRandDoubleParamVal(SpartaABC_options::_priorDistTypeIR, SpartaABC_options::_minDRVal, SpartaABC_options::_maxDRVal);
		}

		if (!model_choice[0]){
			lengthDistParamsDeletion = lengthDistParamsInsertion;
		}
		if (!model_choice[1]){
		...
		//SIMULATION!!!
		vector<double> summStatsSim=simulateSequencesAndReturnSummaryStatistics(randRootLength, 
			lengthDistParamsInsertion, lengthDistParamsDeletion,
		...
		else
		{
			...
			if (euclideanDist < currentCutOffValue)
			{
				...
				for(auto & elem : lengthDistParamsInsertion) {
					tmp.push_back(elem);
				}
				for(auto & elem : lengthDistParamsDeletion) {
					tmp.push_back(elem);
				}
		...
		resFile <<euclideanDist<<"\t" <<randRootLength << '\t';
		for(auto & elem : lengthDistParamsInsertion) {
			resFile << elem << '\t';
		}
		for(auto & elem : lengthDistParamsDeletion) {
		...
		resFile << randInsertRatio << '\t' << randDeletionRatio;
		for (size_t l = 0; l < summStatsSim.size(); ++l) {
			resFile << "\t" << summStatsSim[l];
		}
		resFile << "\n";
	}
	delete distribtutionTable;
